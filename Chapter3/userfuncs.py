...
def identify_least_significant_feature(results, alpha=0.05):
  ...
  if highest_pvalue > alpha:
    ...
    print("We find the least significant variable in this model is " + variable + " with a p-value of " + str(highest_pvalue) + " and a coefficient of " + str(coeff))
    print("Using the backward methodology, we drop " + variable + " from the new model")
  else:
...
def identify_highest_VIF_feature(vifdf, threshold=5):
  ...
  if highest_vif > threshold:
    ...
    print("We find the highest VIF in this model is " + variable + " with a VIF of " + str(highest_vif))
